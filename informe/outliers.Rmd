...
```{r}
dat<-data2$MaxCPC#summary(dat)

p<-as.vector(fitdistr(dat,"exponential")$estimate)
fitexp <- fitdist(dat,"exp")
plot(fitexp)

#Prueba de Kolmogorov-smirnov y Test de Wilconxo
...
datos<-data.frame(dat,
                  pois=ppois(dat,lambda = p1),
                  geom=pgeom(dat,prob=p2 ),
                  nbin=pnbinom(dat,size=p3[1],mu=p3[2]) )
