...
Estimaremos los parámetros de las funciones supuestas con el comando `fitdistr()` y `fitdist()`, estas funciones me dan los mismos valores, solo que hay funciones que no estan incluidas en ambos. Luego de la estimación de los parámetros se aplicara una prueba de Kolmogórov smirnov y un test de wilconxon para comprobar que los datos se distribuyen como la función encontrada.
...
En este caso se observa mejor ajuste en el histograma y la distribución de los datos, aunque se rechaza la hipótesis nula en la prueba de Kolmogórov se acepta para el caso de la prueba de Wilconxon, entonces podemos decir que estos datos se distribuyen Gamma. Veamos finalmente que pasa si la aproximamos con una densidad Weibull:
...
Mediante el diagrama de caja sin los outliers vemos un comportamiento poco asimetríco, con más peso sobre los valores mayores a la mediana, y esto es lo que se observa con las distribución aproximada. En el diagrama de caja de los datos completos no estaba muy claro el comportamiento de los datos, solo se observaba que estaban muy proximos a cero.
...
En este caso se rechaza la prueba de Kolmogórov, pero se acepta la hipótesis nula de la prueba de Wilconxo, por lo tanto podemos decir que la variable aleatoria asociada a los `CLicks` se distribuye binomial negativa.
...
En todas las distribuciones se rechaza la hipótesis nula en ambas pruebas, por lo tanto no podemos afirmar si estos datos se distribuyen Poisson, Geometríca o Binomial Negativa.
...
Se acepta la prueba de Wilcoxon para el caso de la Binomial Negativa, por lo tanto podemos decir que estos datos se distribuyen de esa manera.
...
Notemos que los límites del intervalo de confianza cambia es más pequeño que de los datos originales, pero a pesar de esto en todos los casos se rechaza la hipótesis nula con ambas pruebas.
...
Por lo gráficos observamos y los resultados obtenidos en las pruebas realizadas concluimos que la distribución Beta es la que mejor se aproxima a esta variable.
...
Aunque an todas las pruebas se rechazo la hipótesis nula. Veamos que obtenemos cuando sacamos los outliers de estos datos:
