...
Con este valor podemos decir que el modelo parece explicar `r R2*100`% la variable `Costo`, para reafirmar estó, estudiaremos las premisas del modelo, es decir que los residuales tengan media cero y varianza constaste, si probamos que los residuales son normales tendriamos un ruido Gaussiano.
...
Tomando en cuenta los resultados obtenidos en el Análisis de Componentes Principales, y por la naturaleza de los datos, realizamos un estudio para ajustar un modelo de regresión multiple a la variable `Ventas` filtrando los datos donde las impresiones eran mayores a cero, los clicks mayores a cero y las ventas mayores a cero, ver documento "correlación.pdf", en el cual obtuvimos que el modelo que mejor se ajusta a esta variable es:
...
Donde el valor de $R^2=45$ y los residuales se comportan normales, para más detalle ver documento anexo.
