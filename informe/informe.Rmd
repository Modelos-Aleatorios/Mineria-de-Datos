...
```{r}
mod.costoc<-lm(Costc ~ MaxCPC+Clicks+Impr+CTR+AvgCPC+AvgPos+Qscore+Ventas+Costo+Convr, data=data)
summary(mod.costoc)
```

```{r}
mod.costo<-lm(Costo ~ MaxCPC+Clicks+Impr+CTR+AvgCPC+AvgPos+Qscore+Ventas+Costc+Convr, data=data)
summary(mod.costo)
```
...
```{r}
mod.ventas<-lm(Ventas ~ MaxCPC+Clicks+Impr+CTR+AvgCPC+Costo+AvgPos+Qscore+Costc+Convr, data=data)
summary(mod.ventas)
```
```{r}
anova(mod.ventas)
